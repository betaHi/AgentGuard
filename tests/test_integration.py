...
from agentguard.analysis import analyze_bottleneck, analyze_context_flow, analyze_failures, analyze_flow
...
def test_e2e_decorator_workflow():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        init_recorder(task="Integration Test", trigger="test", output_dir=f"{tmpdir}/traces")
...
def test_e2e_context_manager_workflow():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        init_recorder(task="CM Test", output_dir=f"{tmpdir}/traces")
...
def test_e2e_middleware_workflow():
    ...
    init_recorder(task="Middleware Test")
...
def test_e2e_analysis_workflow():
    ...
    init_recorder(task="Analysis Test")
    orchestrator()
    trace = finish_recording()
...
def test_e2e_diff_workflow():
    ...
    with contextlib.suppress(BaseException): v2_agent()
...
def test_e2e_health_report():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for i in range(5):
            ...
            def agent(_i=i):
                if _i == 4:  # last run fails
                    raise RuntimeError("crash")
                return {"ok": True}

            init_recorder(task=f"run-{i}", output_dir=str(traces_dir))
            with contextlib.suppress(BaseException): agent()
            finish_recording()
...
def test_e2e_web_report():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        init_recorder(task="Web Report Test", output_dir=str(traces_dir))
        agent()
        finish_recording()
...
def test_e2e_query_workflow():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for task, trigger, should_fail in [
            ...
            def agent(_sf=should_fail):
                if _sf:
                    raise RuntimeError("fail")
                return {"ok": True}

            init_recorder(task=task, trigger=trigger, output_dir=str(traces_dir))
            with contextlib.suppress(BaseException): agent()
            finish_recording()
...
def test_e2e_evolve_workflow():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for _ in range(3):
            init_recorder(task="Evolve Test", output_dir=f"{tmpdir}/traces")
            coord()
            trace = finish_recording()
...
def test_e2e_full_cycle():
    ...
    def worker_b(task):
        try: return broken()
        except Exception: return {"fallback": True}
