"""Trace generator — generate synthetic traces for benchmarking and testing.

Create realistic random traces with configurable:
- Number of agents, tools, handoffs
- Failure rates
- Duration ranges
- Cost/token distributions
"""

from __future__ import annotations

import random

from agentguard.builder import TraceBuilder
from agentguard.core.trace import ExecutionTrace

_AGENT_NAMES = [
    "researcher", "analyst", "writer", "reviewer", "editor",
    "coder", "tester", "deployer", "monitor", "planner",
    "collector", "parser", "enricher", "summarizer", "validator",
]

_TOOL_NAMES = [
    "web_search", "db_query", "file_read", "api_call", "cache_lookup",
    "parser", "formatter", "validator", "notifier", "logger",
]


def _add_agent_with_children(
    builder: "TraceBuilder", name: str, dur: float,
    failure_rate: float, tools_per_agent: int,
    include_llm_calls: bool, include_costs: bool,
    input_data: dict | None,
) -> None:
    """Add an agent span with tools and optional LLM call."""
    failed = random.random() < failure_rate
    tokens = random.randint(500, 5000) if include_costs else None
    cost = tokens * 0.00003 if tokens else None

    builder.agent(
        name, duration_ms=dur,
        status="failed" if failed else "completed",
        error=f"Random failure in {name}" if failed else None,
        output_data={"result": f"output_from_{name}", "items": list(range(random.randint(1, 5)))},
        input_data=input_data, token_count=tokens, cost_usd=cost,
    )

    for _ in range(max(0, random.randint(0, tools_per_agent * 2))):
        tool = random.choice(_TOOL_NAMES)
        tool_failed = random.random() < failure_rate * 0.5
        builder.tool(
            tool, duration_ms=random.uniform(100, dur / 2),
            status="failed" if tool_failed else "completed",
            error=f"{tool} error" if tool_failed else None,
            retry_count=random.randint(0, 2) if random.random() < 0.2 else 0,
        )

    if include_llm_calls and random.random() < 0.7:
        llm_tokens = random.randint(500, 3000)
        builder.llm_call(
            f"llm_{name}", duration_ms=random.uniform(1000, dur * 0.8),
            token_count=llm_tokens, cost_usd=llm_tokens * 0.00003,
        )

    builder.end()


def generate_trace(
    task: str = "synthetic_pipeline",
    agents: int = 3,
    tools_per_agent: int = 1,
    handoffs: bool = True,
    failure_rate: float = 0.1,
    min_duration_ms: float = 500,
    max_duration_ms: float = 10000,
    include_llm_calls: bool = True,
    include_costs: bool = True,
    seed: int | None = None,
) -> ExecutionTrace:
    """Generate a random but realistic trace for testing."""
    if seed is not None:
        random.seed(seed)
    builder = TraceBuilder(task)
    agent_names = random.sample(_AGENT_NAMES, min(agents, len(_AGENT_NAMES)))
    prev = None
    for i, name in enumerate(agent_names):
        dur = random.uniform(min_duration_ms, max_duration_ms)
        if handoffs and prev:
            builder.handoff(prev, name, context_size=random.randint(100, 5000))
        _add_agent_with_children(
            builder, name, dur, failure_rate, tools_per_agent,
            include_llm_calls, include_costs,
            {"query": f"input_for_{name}"} if i > 0 else None,
        )
        prev = name
    return builder.build()


def generate_batch(
    count: int = 10,
    **kwargs,
) -> list[ExecutionTrace]:
    """Generate multiple synthetic traces."""
    # Remove seed from kwargs before passing
    kwargs.pop("seed", None)
    return [generate_trace(task=f"synthetic_{i}", seed=i, **kwargs)
            for i in range(count)]
