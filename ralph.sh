...
STORYEOF
    ...
    # Add explicit "this does NOT exist" verification to the message
    STORY_KEYWORDS=$(echo "$NEXT_STORY" | tr ' ' '
' | grep -E "^[a-z]" | head -5 | tr '
' '|' | sed 's/|$//')
    EXISTING_CODE=$(grep -rn "$STORY_KEYWORDS" agentguard/ --include="*.py" 2>/dev/null | head -3)
...
VERIFICATION: I searched the codebase and $(if [ -n \"$EXISTING_CODE\" ]; then echo \"found related code but the SPECIFIC feature in this story may not be complete\"; else echo \"this feature does NOT exist yet — you must implement it from scratch\"; fi). 
